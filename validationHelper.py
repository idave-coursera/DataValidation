...
class Table:
  ...
  time_travel_ts: UTC
  """
  def __init__(self, name, pkey=[], ts_col=None, from_time_stamp=None, time_travel_ts=None, to_time_stamp=None, ss=None, filter=None):
    self.name = name
    self.size = 0
...
class Validator:
  def __init__(self, edw_table: Table, eds_table: Table,ss):
    ...
    print(edw_table.schema.merge(eds_table.schema, indicator=True, how='left').query('_merge == "left_only"').drop('_merge', axis=1))

    print(f"\nThe following columns have been excluded from the EDS table:")
    print(eds_table.schema.merge(edw_table.schema, indicator=True, how='left').query('_merge == "left_only"').drop('_merge', axis=1))
  ...
  def run_column_spot_check(self, sample_pct,truncate_ts = False):
    ...
    print(f"WARN: With a sample size of {sample_pct}% ({limit_val} records), running this function may take up to {total_memory_footprint_in_mb} MB. Verify that your compute configuration has more than enough memory to handle this workload, otherwise lower the sample size.")
    ...
    for row in self.shared_schema.itertuples():
      ...
      if truncate_ts and data_type == 'timestamp' :
        diffs = cols[pd.to_datetime(cols[col_name + '__edw']).dt.floor('S') != pd.to_datetime(cols[col_name + '__eds']).dt.floor('S')]
      else :
